...
Translates natural language research goals into executable DAG specifications using the Anthropic API. Constructs a system prompt with available simulation types and their parameter schemas, calls the API, parses the JSON response, and validates the result through the DAG validation pipeline before creation.
...
| Anthropic SDK | LLM integration | Direct API access for structured output; no LangChain abstraction overhead |
