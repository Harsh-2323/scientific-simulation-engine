...
async def decompose_dag(
    ...
    Calls an LLM (via any OpenAI-compatible API) to generate a simulation plan,
    validates the structure, and optionally creates the DAG (unless dry_run=True).
    ...
    try:
        dag_spec, model_used, latency_ms, quality_warnings = await decompose_instruction(
            request.instruction,
    ...
    # Validate the generated spec (structural) + merge quality warnings
